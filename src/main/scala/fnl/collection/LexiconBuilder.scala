...
private class LexiconBuilder[T <% Ordered[T]] extends mutable.Builder[Seq[T], Lexicon[T]] {
  ...
  /** Get the suffix and state for `word` at `state` that is at the longest common prefix of
    * `word` in the Lexicon (see [1], Algorithm 1, func common_prefix).
    */
  ...
  /** Build a linear stack of all `parent -symbol-> child` transitions starting at `parent` by
    ...
    */
  @tailrec private def latestTransitions(parent: Int,
                                         stack: List[(Int, T, Int)]): List[(Int, T, Int)] = {
  ...
  /** Register new states or expand transitions from existing states to add the latest
    * unregistered suffix - defined as path.substring(offset) - to the digraph.
    *
